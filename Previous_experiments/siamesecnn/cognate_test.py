...
from keras.layers import merge, Dense, Dropout, Flatten, Input, Lambda, Masking, GRU
...
def make_cognates_pairs(d):
    ...
    for concept in d:
        for i1, i2 in it.combinations(d[concept], r=2):
            ...
            if l1 != l2:
                if g1 == g2:
                    cog_dictionary[l1][l2] += 1
    return cog_dictionary
'''
...
for line in f:
    ...
    d[globalid].append((asjp_word, cogid, lang))
...
###############Uncomment the convolution part to replicate########
##, activation="relu"
x = Convolution2D(10, n_dim, 2, input_shape = (1, n_dim, max_word_len))(word_input)##Uncomment for COLING paper
...
######Trying a GRU#############
#out = GRU(8, dropout_W=0.2, dropout_U=0.2)(word_input)
...
model.fit([train_1, train_2, train_lang], train_labels, nb_epoch=nb_epoch, batch_size=batch_size, validation_data=([test_1, test_2, test_lang], test_labels), callbacks=[early_stopping])
...
for i in range(len(test_pairs)):
    print('pair = %s, Predicted = %s' % (test_pairs[i], round(te_pred[i])))
